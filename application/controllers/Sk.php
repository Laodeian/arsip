...
class Sk extends CI_Controller
{
    ...
    public function hapus_multiple()
    {
        if ($this->input->is_ajax_request() == true) {
            ...
            if ($hapusData == true) {
                $msg = [
                    'status' => TRUE,
                    'msg' => "$jmlData data surat Keluar dihapus."
                ];
            } else {
                $msg  = [
                    'status' => FALSE,
    ...
    public function tambah_sk()
    {
        if ($this->input->is_ajax_request() == TRUE) {
            ...
            'no_agenda',
            'No. Agenda',
            'required|regex_match[/^[A-Za-z0-9\/\-]+$/]|is_unique[surat_keluar.no_agenda]');
    ...
    public function ubah_sk()
    {
        if ($this->input->is_ajax_request() == TRUE) {
            ...
            if ($sk['no_agenda'] == $noAgenda) {
                ...
                $ruleAgenda = 'required|regex_match[/^[A-Za-z0-9\/\-]+$/]|is_unique[surat_keluar.no_agenda]';
            }
