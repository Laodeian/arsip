...
class Sm extends CI_Controller
{
    ...
    public function tambah()
    {
        $data = [
            ...
            'judul' => 'Tambah Surat Masuk',
                // =================================================
            // TAMBAHAN UNTUK MEMBUAT NOMOR AGENDA OTOMATIS
            // =================================================
            'no_agenda' => $this->sm->getNoAgenda()
            // =================================================
    ...
    public function tambah_sm()
    {
            ...
            'no_agenda',
            'No. Agenda',
            'required|is_unique[surat_masuk.no_agenda]|regex_match[/^[A-Za-z0-9\/\-]+$/]');//diubah
        $this->form_validation->set_rules('no_surat', 'No. Surat', 'required');
        $this->form_validation->set_rules('pengirim', 'Pengirim', 'required');
        $this->form_validation->set_rules('isi', 'Isi Perihal', 'required|max_length[300]');
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($upload_file) {
                ...
                $config['max_size'] = 5000; //ubah size apload file
    ...
    public function ubah_sm()
    {
        if ($this->input->is_ajax_request() == TRUE) {
            ...
            //ubah angka numerik/kombinasi huruf dan angka
            if ($sm['no_agenda'] == $noAgenda) {
                $ruleAgenda = 'required|regex_match[/^[A-Za-z0-9\/\-]+$/]';
            } else {
                $ruleAgenda = 'required|regex_match[/^[A-Za-z0-9\/\-]+$/]|is_unique[surat_masuk.no_agenda]';
            }
            $this->form_validation->set_rules('no_agenda', 'No. Agenda', $ruleAgenda);
            $this->form_validation->set_rules('no_surat', 'No. Surat', 'required');
            $this->form_validation->set_rules('pengirim', 'Pengirim', 'required');
            $this->form_validation->set_rules('isi', 'Perihal', 'required|max_length[300]');
            $this->form_validation->set_rules('tgl_surat', 'Tanggal Surat', 'required');
            $this->form_validation->set_rules('tgl_diterima', 'Tanggal Diterima', 'required');
