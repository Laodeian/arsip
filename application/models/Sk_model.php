    ...
    public function hapus_multiple($id, $jmlData)
{
    for ($i = 0; $i < $jmlData; $i++) {
        ...
        $row = $this->db->get_where('surat_keluar', ['id' => $id[$i]])->row_array();

        // Jika data ditemukan
        if ($row) {

            // Jika ada file dan file benar-benar ada di folder
            if (!empty($row['file']) && file_exists(FCPATH . 'uploads/' . $row['file'])) {
                unlink(FCPATH . 'uploads/' . $row['file']);
            }
